...
import { Linkedin } from 'react-bootstrap-icons'; // Import Linkedin icon
...
const ApieMus: React.FC = () => {
  ...
  <div key={index} className="flex flex-col items-start text-left md:flex-row md:items-start md:space-x-12">
    <Card className="w-full md:w-1/2 flex-shrink-0">
      <article className="relative w-full h-full p-6 group bg-[rgba(28, 28, 30, 0.8)] hover:bg-[rgba(44, 44, 46, 0.8)]">
        <Image src={section.image} alt={section.title} width={section.width} height={section.height} className="mb-4" /> {/* Add image above the title */}
