...
export default function Home() {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center w-screen h-screen overflow-hidden bg-black">
      ...
      {/* Desktop Navigation */}
      <nav className="hidden md:flex justify-center z-10 w-full absolute top-0 left-0 py-6">
        <ul className="flex items-center space-x-12">
          {navigation.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="text-lg text-zinc-500 hover:text-white transition-colors duration-300"
              >
                {item.name}
              </Link>
            </li>
      ...
      <div className="flex flex-col items-center justify-center z-10">
        ...
        />
        <h1
          className="text-4xl md:text-9xl text-transparent bg-white bg-clip-text duration-1000 cursor-default text-edge-outline font-display animate-title"
